...
from dodal.devices.electron_analyser.region.specs_region import SpecsRegion
...
class SpecsAnalyserDriverIO(
    ...
    def __init__(
        ...
        super().__init__(
            prefix=prefix,
            acquisition_mode_type=SpecsAcquisitionMode,
    ...
    async def _set_region(self, ke_region: SpecsRegion[TLensMode, TPsuMode]):
        ...
        if ke_region.acquisition_mode == SpecsAcquisitionMode.FIXED_TRANSMISSION:
            await self.energy_step.set(ke_region.energy_step)

        if ke_region.acquisition_mode == SpecsAcquisitionMode.FIXED_ENERGY:
