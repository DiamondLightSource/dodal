...
class OAV(StandardReadable, Flyable):
    def __init__(self, prefix: str, params: OAVConfigParams, name: str = "") -> None:
        ...
        self.redis_client = redis.StrictRedis(
            host="i04-control.diamond.ac.uk", password="", db=7
        )
            ...
            async with session.get(self.stream_url) as response:
                while True:
                    ...
                    if line.startswith(b"\xff\xd8"):
                        frame_data = line + await response.content.readuntil(
                            b"\xff\xd9"
                        )
                        ...
                        LOGGER.info("Converted to image")

                        numpydata = np.asarray(img)
                        LOGGER.info("Converted to np array")
                        await self.redis_client.hset(
                            "test-image", image_uuid, pickle.dumps(numpydata)
                        )
                        LOGGER.info(
