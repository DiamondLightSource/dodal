...
class Xspress3Mini(Device):
    # todo: change how this uses prefixes

    # Assume only one channel for now
    channel_1 = Component(Xspress3MiniChannel, f":C1")

    pv_erase: EpicsSignal = Component(
        EpicsSignal, f"{prefix}:ERASE"
    )  # doErase function sets this PV to 1, which does the erase, IntergerFromEnumPV
    pv_get_max_num_channels = Component(
        EpicsSignalRO, f"{prefix}:MAX_NUM_CHANNELS_RBV"
    )  # Using get on this PV should give the max number of channels, ReadOnlyIntegerPV
    pv_sca5_update_arrays_mini_list: list[EpicsSignalRO] = [
        channel_1.pv_sca5_update_arrays_mini
    ]  # These are read and write PV's, one element of list for each channel
    ...
    #         Component(EpicsSignalRO, f"{prefix}:C{i}_SCAS:TS:TSAcquire")
    ...
    pv_acquire: EpicsSignal = (
        Component(EpicsSignal, f"{prefix}:Acquire"),
    ...
    pv_get_roi_calc_mini: EpicsSignal = Component(
        EpicsSignal, f"{prefix}:MCA1:Enable_RBV"
    )  # only .get() is used on this

    NUMBER_ROIS_DEFAULT = 6  # This might need to be changed

    pv_roi_size_list: list[EpicsSignal] = [
        channel_1.pv_roi_size
    ]  # one element for each channel

    pv_roi_llm_list: list[EpicsSignal] = [channel_1.pv_roi_llm]
    ...
    set_trigger_mode_mini: EpicsSignal = Component(
        EpicsSignal, f"{prefix}:TriggerMode"
    )  # enumPV, putwait is used on this within a set_trigger_mode function

    pv_get_trig_mode_mini: EpicsSignalRO = Component(
        EpicsSignalRO, f"{prefix}:TriggerMode_RBV"
    )  # Only get is used on this

    # roi_llm and roi_size are then used to get the roi limits in a get_roi_limits function

    pv_roi_start_x: EpicsSignal = Component(EpicsSignal, f"{prefix}:ROISUM1:MinX")
    pv_roi_size_x: EpicsSignal = Component(EpicsSignal, f"{prefix}:ROISUM1:SizeX")
    ...
    pv_acquire_time: EpicsSignal = Component(
        EpicsSignal, f"{prefix}:AcquireTime"
    )  # PV<double>


my_det = Xspress3Mini("BL03I-EA-XSP3-01", name="test_fluorence")
