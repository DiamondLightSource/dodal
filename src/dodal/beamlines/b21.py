...
@devices.factory()
def wbscam(path_provider: PathProvider) -> AravisDetector:
...
# The linkam is only used for some experiments
@devices.factory(skip=True)
def linkam() -> Linkam3:
...
@devices.factory()
def flame_spectrometer() -> FlameSpectrometer:
    return FlameSpectrometer(f"{PREFIX.beamline_prefix}-EA-FLAME-01:")
