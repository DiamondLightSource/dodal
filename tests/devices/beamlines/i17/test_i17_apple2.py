...
async def mock_id_controller(
    ...
    mock_hase_energy_motor_lutt = EnergyMotorLookup()
    mock_hase_energy_motor_lutt.find_value_in_lookup_table = MagicMock(return_value=7.5)
    with init_devices(mock=True):
        mock_id_controller = I17Apple2Controller(
            apple2=mock_apple2,
            gap_energy_motor_lut=mock_gap_energy_motor_lutt,
            hase_energy_motor_lut=mock_hase_energy_motor_lutt,
        )
