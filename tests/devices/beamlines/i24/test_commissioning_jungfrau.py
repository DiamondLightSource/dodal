...
@pytest.fixture
def jungfrau(tmpdir: Path) -> CommissioningJungfrauDetector:
    with init_devices(mock=True):
        name = StaticFilenameProvider("jf_out")
        path = AutoIncrementingPathProvider(name, PurePath(tmpdir))
        detector = CommissioningJungfrauDetector("", "", path, "")
...
async def test_jungfrau_with_temporary_writer(
    ...
    jungfrau.writer_logic.path_provider = MagicMock()
    trigger_info = TriggerInfo(livetime=1e-3, collections_per_event=5)
    await jungfrau.prepare(trigger_info)
    assert await jungfrau.writer.frame_counter.get_value() == 0
    assert (
        await jungfrau.writer.expected_frames.get_value()
        == trigger_info.number_of_exposures
    )
    ...
    async def _do_fake_writing():
        for frame in range(1, 5):
            set_mock_value(jungfrau.writer.frame_counter, frame)
            assert not status.done
        set_mock_value(jungfrau.writer.frame_counter, 5)

    await asyncio.gather(status, _do_fake_writing())
    jungfrau.writer_logic.path_provider.assert_called_once()
...
async def test_jungfrau_error_when_writing_to_existing_file(tmp_path: Path):
    ...
    with init_devices(mock=True):
        jungfrau = CommissioningJungfrauDetector("", "", path, "")
    set_mock_value(jungfrau.writer.writer_ready, 1)
    with pytest.raises(FileExistsError):
        await jungfrau.prepare(TriggerInfo(livetime=1e-3, collections_per_event=5))
