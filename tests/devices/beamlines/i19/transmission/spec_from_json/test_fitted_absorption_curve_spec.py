...
WITH_OR_WITHOUT_RESIDUALS: Final[tuple[list[float], ...]] = ([], [40.9271, -21.6924, 7.5198, -12.51, 8.87e-2, -5.39e-4, 4.796e-5])

@pytest.mark.parametrize(
    "residuals",
    WITH_OR_WITHOUT_RESIDUALS
)
@pytest.mark.parametrize("roll_off", (-2.983, -2.83, -2.72,))
@pytest.mark.parametrize(
    "scaling_constant", (
        3123.4,
        7812.9,
...
def test_that_non_numerical_scaling_constants_rejected_by_fitted_absorption_curve_spec(
    non_numerical_scaling_constant: Any,
    residuals: list[float]
) -> None:
    d: dict[str, Any] = {
...
UNPHYSICAL_ROLL_OFF_EXPONENTS: Final[list[float]] = [
    1.2, 2.96, -1.5, -4.5, -296, -1, pi, 1, 4, 44.8, 200004, -1289,
]
