import pytest
from daq_config_server.models.lookup_tables.insertion_device import (
    parse_i09_hu_undulator_energy_gap_lut,
)

from dodal.testing.fixtures.config_client import ConverterDict
from tests.devices.beamlines.i09_1_shared.test_data import TEST_HARD_UNDULATOR_LUT


@pytest.fixture
def mock_config_client_data() -> ConverterDict:
    return {TEST_HARD_UNDULATOR_LUT: parse_i09_hu_undulator_energy_gap_lut}
