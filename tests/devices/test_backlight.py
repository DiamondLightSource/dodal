...
async def test_when_backlight_moved_out_it_switches_off(
    mock_sleep: AsyncMock, fake_backlight: Backlight, run_engine: RunEngine
):
    run_engine(bps.mv(fake_backlight, InOut.OUT))
...
async def test_when_backlight_moved_in_it_switches_on(
    mock_sleep, fake_backlight: Backlight, run_engine: RunEngine
):
    run_engine(bps.mv(fake_backlight, InOut.IN))
