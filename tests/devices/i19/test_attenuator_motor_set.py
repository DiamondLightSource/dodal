...
def test_that_attenuator_position_demand_can_be_created_with_only_one_wedge():
    ...
    position_demand = AttenuatorPositionDemand(
        wedge_position_demands, wheel_position_demands
    )
...
def test_that_attenuator_position_demand_with_only_one_wedge_provides_expected_rest_format():
    ...
    restful_encoding = position_demand.restful_format()
    assert restful_encoding == "{'y': 14.9}"
...
def test_that_attenuator_position_demand_with_only_one_wheel_provides_expected_rest_format():
    ...
    rest = position_demand.restful_format()
    assert rest == "{'w': 6}"
...
def test_that_attenuator_position_demand_triplet_can_be_created():
    ...
    position_demand = AttenuatorPositionDemand(
        standard_wedge_position_demand, standard_wheel_position_demand
    )
    assert position_demand is not None
...
def test_that_attenuator_position_raises_error_when_discrete_and_continuous_demands_overload_axis_label():
    ...
    try:
        AttenuatorPositionDemand(wedge_position_demands, wheel_position_demands)
        raise NotImplementedError("Did not observe expected error")
    except ValueError as expected:
        assert (
            str(expected)
            == "Clashing keys in common between wheel and wedge for attenuation position demand: v"
        )
