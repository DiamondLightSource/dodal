...
@pytest.fixture
def aperture_in_medium_pos(fake_aperture_scatterguard: ApertureScatterguard,
                           aperture_positions: AperturePositions):
    fake_aperture_scatterguard.load_aperture_positions(aperture_positions)
    fake_aperture_scatterguard.aperture.x.user_setpoint.sim_put(aperture_positions.MEDIUM[0])
    fake_aperture_scatterguard.aperture.y.user_setpoint.sim_put(aperture_positions.MEDIUM[1])
    fake_aperture_scatterguard.aperture.z.user_setpoint.sim_put(aperture_positions.MEDIUM[2])
    fake_aperture_scatterguard.aperture.x.user_readback.sim_put(aperture_positions.MEDIUM[1])
    fake_aperture_scatterguard.aperture.y.user_readback.sim_put(aperture_positions.MEDIUM[1])
    fake_aperture_scatterguard.aperture.z.user_readback.sim_put(aperture_positions.MEDIUM[1])
    fake_aperture_scatterguard.scatterguard.x.user_setpoint.sim_put(aperture_positions.MEDIUM[3])
    fake_aperture_scatterguard.scatterguard.y.user_setpoint.sim_put(aperture_positions.MEDIUM[4])
    fake_aperture_scatterguard.scatterguard.x.user_readback.sim_put(aperture_positions.MEDIUM[3])
    fake_aperture_scatterguard.scatterguard.y.user_readback.sim_put(aperture_positions.MEDIUM[4])
...
def aperture_positions():
    aperture_positions = AperturePositions.from_gda_beamline_params(
        {"miniap_x_LARGE_APERTURE": 2.389, "miniap_y_LARGE_APERTURE": 40.986, "miniap_z_LARGE_APERTURE": 15.8,
         "sg_x_LARGE_APERTURE": 5.25, "sg_y_LARGE_APERTURE": 4.43, "miniap_x_MEDIUM_APERTURE": 2.384,
         "miniap_y_MEDIUM_APERTURE": 44.967, "miniap_z_MEDIUM_APERTURE": 15.8, "sg_x_MEDIUM_APERTURE": 5.285,
         "sg_y_MEDIUM_APERTURE": 0.46, "miniap_x_SMALL_APERTURE": 2.430, "miniap_y_SMALL_APERTURE": 48.974,
         "miniap_z_SMALL_APERTURE": 15.8, "sg_x_SMALL_APERTURE": 5.3375, "sg_y_SMALL_APERTURE": -3.55,
         "miniap_x_ROBOT_LOAD": 2.386, "miniap_y_ROBOT_LOAD": 31.40, "miniap_z_ROBOT_LOAD": 15.8,
         "sg_x_ROBOT_LOAD": 5.25, "sg_y_ROBOT_LOAD": 4.43, })
...
def test_aperture_scatterguard_select_bottom_moves_sg_down_then_assembly_up(
    ...
    expected_calls = [("_mock_sg_x", (5.3375,)),
                      ("_mock_sg_y", (-3.55,)),
...
def test_aperture_scatterguard_select_top_moves_assembly_down_then_sg_up(
    ...
    actual_calls = call_logger.mock_calls
    expected_calls = [("_mock_ap_x", (2.389,)),
                      ("_mock_ap_y", (40.986,)),
                      ("_mock_ap_z", (15.8,)),
                      lambda call: call[0].endswith("__and__().wait"),
                      ("_mock_sg_x", (5.25,)),
    ...
    for i_expected in range(0, len(expected_calls)):
        try:
            ...
            if isinstance(expected, tuple):
                ...
                i_matches = [i for i in range(i_actual, len(actual_calls)) if expected(actual_calls[i])]
                if i_matches:
                    i_actual = i_matches[0]
                else:
                    raise ValueError("Couldn't find call matching predicate")
        except ValueError:
            assert False, f"Couldn't find call #{i_expected}: {expected_calls[i_expected]}"
