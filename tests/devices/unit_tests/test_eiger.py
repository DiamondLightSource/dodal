...
@pytest.mark.parametrize(
    "energy_ev, request_energy, is_energy_change",
    [
        (100.0, 100.0, False),
        (100.0, 200.0, True),
        (100.0, 50.0, True),
...
def test_detector_threshold(
    ...
    when(fake_eiger.cam.photon_energy).get().thenReturn(energy_ev)
...
def test_unsuccessful_true_roi_mode_change_results_in_callback_error(
    ...
    unwrapped_funcs = [
        ...
        lambda: fake_eiger.set_detector_threshold(
            energy=fake_eiger.detector_params.energy_ev
        ),
    ]
