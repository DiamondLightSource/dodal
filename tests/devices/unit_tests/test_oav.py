...
def test_snapshot_trigger_handles_request_with_bad_status_code_correctly(
    mock_get, mock_oav: OAV
):
    response = Response()
    response.status_code = 404
    mock_get.return_value = response

    st = mock_oav.grid_snapshot.trigger()
...
def test_snapshot_trigger_loads_correct_url(
    mock_image: MagicMock, mock_get: MagicMock, mock_oav: OAV
):
    st = mock_oav.grid_snapshot.trigger()
    st.wait()
...
def test_snapshot_trigger_saves_to_correct_file(
    mock_open: MagicMock, mock_get, mock_oav
):
...
def test_given_directory_not_existing_when_snapshot_triggered_then_directory_created(
    mock_os, mock_open: MagicMock, mock_get, mock_oav
):
...
def test_snapshot_trigger_applies_current_microns_per_pixel_to_snapshot(
    ...
    expected_mpp_x = mock_oav.parameters.micronsPerXPixel
    expected_mpp_y = mock_oav.parameters.micronsPerYPixel
    with patch.object(image, "save"):
        st = mock_oav.grid_snapshot.trigger()
        st.wait()
        assert mock_oav.grid_snapshot.microns_per_pixel_x.get() == expected_mpp_x
        assert mock_oav.grid_snapshot.microns_per_pixel_y.get() == expected_mpp_y
...
def test_when_zoom_1_then_flat_field_applied(mock_oav: OAV, RE: RunEngine):
    RE(bps.abs_set(mock_oav.zoom_controller, "1.0x"))
    assert mock_oav.grid_snapshot.input_plugin.get() == "PROC"


def test_when_zoom_not_1_then_flat_field_removed(mock_oav: OAV, RE: RunEngine):
    RE(bps.abs_set(mock_oav.zoom_controller, "10.0x"))
    assert mock_oav.grid_snapshot.input_plugin.get() == "CAM"
...
def test_when_zoom_is_externally_changed_to_1_then_flat_field_not_changed(
    ...
    mock_oav.grid_snapshot.input_plugin.sim_put("CAM")  # type: ignore

    mock_oav.zoom_controller.level.sim_put("1.0x")  # type: ignore
    assert mock_oav.grid_snapshot.input_plugin.get() == "CAM"
...
def test_get_beam_position_from_zoom_only_called_once_on_multiple_connects(
    ...
    with (
        ...
        mock_oav.zoom_controller.level.sim_put("2.0x")  # type: ignore
