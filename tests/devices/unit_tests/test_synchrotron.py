...
READINGS = [
    constants.SYNCHR_RING_CURRENT,
    constants.SYNCHR_USER_COUNTDOWN,
    constants.SYNCHR_START_COUNTDOWN,
    constants.SYNCHR_END_COUNTDOWN,
]
CONFIGS = [constants.SYNCHR_BEAM_ENERGY, constants.SYNCHR_MODE]
...
EXPECTED_DESCRIBE_RESULT = f"""{{
  "{READ_SIGNALS[0]}":{{
    ...
    "{DESCRIPTION_FIELDS[1]}": "{constants.SYNCHR_NUMBER}",
...
EXPECTED_DESCRIBE_CONFIG_RESULT = f"""{{
  ...
  "{CONFIG_SIGNALS[1]}":{{
    ...
    "{DESCRIPTION_FIELDS[1]}": "{constants.SYNCHR_STRING}",
    "{DESCRIPTION_FIELDS[2]}": {EMPTY_LIST}
  }}
}}"""


async def test_synchrotron_read(mock_synchrotron: Synchrotron):
    await _verify(
        mock_synchrotron.read,
        READ_SIGNALS,
        READING_FIELDS,
...
async def test_synchrotron_read_configuration(mock_synchrotron: Synchrotron):
    await _verify(
        mock_synchrotron.read_configuration,
        CONFIG_SIGNALS,
        READING_FIELDS,
...
async def test_synchrotron_describe(mock_synchrotron: Synchrotron):
    await _verify(
        mock_synchrotron.describe,
        READ_SIGNALS,
        DESCRIPTION_FIELDS,
...
async def test_synchrotron_describe_configuration(mock_synchrotron: Synchrotron):
    await _verify(
        mock_synchrotron.describe_configuration,
        CONFIG_SIGNALS,
        DESCRIPTION_FIELDS,
...
async def test_synchrotron_count(RE: RunEngine, mock_synchrotron: Synchrotron):
    docs = []
    RE(count_sim(mock_synchrotron), lambda x, y: docs.append(y))

    assert len(docs) == 4
    assert mock_synchrotron.name in docs[1]["configuration"]
    cfg_data_keys = docs[1]["configuration"][mock_synchrotron.name]["data_keys"]
    for sig, addr in zip(CONFIG_SIGNALS, CONFIG_ADDRESSES):
        assert sig in cfg_data_keys
        dtype = (
            constants.SYNCHR_NUMBER
            if sig == CONFIG_SIGNALS[0]
            else constants.SYNCHR_STRING
        )
    ...
    cfg_data = docs[1]["configuration"][mock_synchrotron.name]["data"]
    ...
    for sig, addr in zip(READ_SIGNALS, READING_ADDRESSES):
        ...
        assert data_keys[sig][DESCRIPTION_FIELDS[1]] == constants.SYNCHR_NUMBER
